...
class arduino_launch(object):
	...
	def Update_Twist(self,msg):
		...
		#print self.angular
		twist_message = "<s, %0.1f, %0.1f>" %(float(self.linear),float(self.angular)) 
	...
	def WriteSerial(self,message):
		...
		rospy.loginfo('i %d : a %d : e %d : r %d : p %d ' %(int(self.linear_in),int(self.left_set_RPM),int(self.left_encoder_value), int(self.left_RPM),int(self.left_out_PWM)))
